...
def index():
    ...
    if request.method == "POST":
        ...
        if selected_symptoms:  # Check if any symptoms were selected
            ...
            for disease, symptoms in Diseases.items():
                ...
                # Calculate percentage (each disease has exactly 5 symptoms)
                if match_count > 0:
                    percentage = int((match_count / 5) * 100)
                    disease_probabilities[disease] = percentage
            ...
            if disease_probabilities:
                ...
                results = [(disease, percentage) for disease, percentage in top_three]
                
                # Get recommendation for top disease
                if top_three:
                    top_disease = top_three[0][0]
                    recommendation = recommendations.get(top_disease, 
                        "Please consult with the school nurse for proper assessment.")
